...
export class DataExcel {
  ...
  static async exportClasses(classesData) {
    try {
      ...
      headerCell.value = 'SMPN 2 Cililin';
      ...
      link.href = url;
      link.download = `Data_Kelas_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
      link.click();
  ...
  static async exportAllStudents(studentsData) {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `Data_Siswa_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
      link.click();
      URL.revokeObjectURL(url);
  ...
  static async exportByJenjang(studentsData, jenjang) {
    ...
    // HEADER
    worksheet.mergeCells('A1:F1');
    worksheet.getCell('A1').value = 'SMPN 2 Cililin';
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Data_Siswa_Kelas_${jenjang}_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
    link.click();
    URL.revokeObjectURL(url);
  }
  ...
  static async exportByKelas(studentsData, kelas) {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Data_Siswa_${kelas}_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
    link.click();
    URL.revokeObjectURL(url);
  }
  ...
  static async exportByFilter(filteredData, selectedKelas, selectedJenjang, selectedGender) {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Data_Siswa_Filter_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
    link.click();
    URL.revokeObjectURL(url);
  }
  ...
  static async exportTeachers(teachersData) {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `Data_Guru_SMPN_2_Cililin_${this.getTahunAjaranAktif()}.xlsx`;
      link.click();
      URL.revokeObjectURL(url);
